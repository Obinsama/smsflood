 ...
 */
var socket = __webpack_require__(29)('http://localhost:9090');
...
var chat = {
    ...
    isValidInputMessage: function isValidInputMessage() {
        return $messagebox.val().length > 0 ? true : false;
    },
    ...
    addChatMessage: function addChatMessage(data) {
        ...
        var $receiverElement = $('<p>').text('To : ' + data.receiver);
        var $messageBodyElement = $('<p>').text('Message : ' + data.message);
        // const $messageElement = $('<li class="message"/>')
        var $messageElement = $(' <li class="messageBody list-group-item-warning">')
    ...
    refresh: function refresh(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            var $receiverElement = $('<p>').text('To : ' + data[i]['receiver']);
            var $messageBodyElement = $('<p>').text('Message : ' + data[i]['message']);
            var $messageElement = '';
            if (data[i]['status'] == 10) {
                $messageElement = $(' <li class="messageBody list-group-item-warning">').append($receiverElement, $messageBodyElement);
            } else if (data[i]['status'] == 11) {
                $messageElement = $(' <li class="messageBody list-group-item-danger">').append($receiverElement, $messageBodyElement);
            } else {
                $messageElement = $(' <li class="messageBody list-group-item-success">').append($receiverElement, $messageBodyElement);
            }
            chat.addMessageElement($messageElement);
        }
    },
    ...
    checknumber: function checknumber(receiver) {
        // if ((inputtxt.match(/^6|2\(?([5-9]{2})\)?[-. ]?\(?[0-9]{3}\)?[-. ]?\(?[0-9]{3}\)?$/))) {
    ...
    addMessageElement: function addMessageElement(element, options) {
        var $element = $(element);
